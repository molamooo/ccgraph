...
void writeStackTrace() {
    ...
    size_t max_message_size = sizeof( backtrace_strings ) * 255;
    ...
    for( size_t i = 1; i < backtrace_size; i++ ) {
        ...
        size_t string_length = strlen( backtrace_strings[i] );
...
void loadConfig( const char* config ) {
    ...
    for( size_t i = 0; i < strlen( settingsBuffer ); i++ ) {
        if( settingsBuffer[i] == '=' ) {
...
#define LOG_FATAL(fmt, ...) simplelogger::writeLog(SIMPLOG_FATAL, fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...) simplelogger::writeLog(SIMPLOG_ERROR, fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...) simplelogger::writeLog(SIMPLOG_INFO, fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...) simplelogger::writeLog(SIMPLOG_WARN, fmt, ##__VA_ARGS__)
#define LOG_DEBUG(fmt, ...) simplelogger::writeLog(SIMPLOG_DEBUG, fmt, ##__VA_ARGS__)
#define LOG_VERBOSE(fmt, ...) simplelogger::writeLog(SIMPLOG_VERBOSE, fmt, ##__VA_ARGS__)